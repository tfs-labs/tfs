#ifndef __TFS_WASMTIME__
#define __TFS_WASMTIME__
...
public:
  ...
  static Result<Module> compile(Engine &engine, std::string_view wat) {
   ...
   static Result<std::vector<uint8_t>> wbl_compile_wat(Engine &engine, std::string_view wat) {
    auto wasm = wat2wasm(wat);
    if (!wasm) {
      return wasm.err();
    }
  ...
  static Result<Module> wbl_import_wasm(Engine &engine, std::string_view wat) {
    std::vector<uint8_t> bytes; 
   // auto bytes = wasm.ok();
    for(int i=0;i<wat.size();i++){
        bytes.push_back(wat[i]);
    }
    return compile(engine, bytes);
  }
...
template<typename T>
   ...
   public:
        ...
        _Result(const T& v){ t=v;flag=true;}
        _Result( TrapError er_){ error.er=er_.message() ;error.er_n=-1;flag=false;}
        _Result(const _Error &er){
            error=er;
            flag=false;
        }
...
public:
  ...
  static Func wrap(Store::Context cx, F f) {
   ...
   // template <typename T> using c_Result = _Result<T>;
   ...
   * > `TypedFunc::call`.
   */
  _Result<std::vector<Val>> call(Store::Context cx,
...
template <typename T> struct SuportType {};

template <> struct SuportType<double> {
    using type = double;
    static void BytesToVar(const std::vector<uint8_t> &bytesArray, type *data) {
        char *tempChar = (char *)data;
        for (int i = 0; i < sizeof(type); i++) {
            tempChar[i] = bytesArray[i];
        }
    }
    static std::vector<uint8_t> VarToBytes(const type *data) {
        std::vector<uint8_t> ret;
        char *tempChar = (char *)data;
        for (int i = 0; i < sizeof(type); i++) {
            ret.push_back(tempChar[i]);
        }
        return ret;
    }
};

template <> struct SuportType<char> {
    using type = char;
    static void BytesToVar(const std::vector<uint8_t> &bytesArray, type *data) {
        char *tempChar = (char *)data;
        for (int i = 0; i < sizeof(type); i++) {
...
template <> struct SuportType<int> {
    using type = int;
    static void BytesToVar(const std::vector<uint8_t> &bytesArray, type *data) {
        char *tempChar = (char *)data;
        for (int i = 0; i < sizeof(type); i++) {
...
template <> struct SuportType<long> {
    using type = long;
    static void BytesToVar(const std::vector<uint8_t> &bytesArray, type *data) {
        char *tempChar = (char *)data;
        for (int i = 0; i < sizeof(type); i++) {
...
template <> struct SuportType<long long> {
...
template <> struct SuportType<std::string> {
    using type = std::string;
    static void BytesToVar(const std::vector<uint8_t> &bytesArray, type *data) {
        for (int i = 0; i < bytesArray.size(); i++) {
            data->push_back(bytesArray[i]);
        }
    }
    static std::vector<uint8_t> VarToBytes(const type *data) {
        std::vector<uint8_t> ret;
...
template <typename T> class Var {
  public:
    using DataType = typename SuportType<T>::type;
    Var() {}
    Var(const std::vector<uint8_t> &bytes) {
        SuportType<T>::BytesToVar(bytes, &_data);
    }

    Var(const DataType &d) { _data = d; }
    Var(const DataType &&d) { _data = std::move(d); }

    Var(const Var &other) { _data = other._data; }
    Var(const Var &&other) { _data = std::move(other._data); }

    Var &operator=(const std::vector<uint8_t> &bytes) {
        SuportType<T>::BytesToVar(bytes, &_data);
    }

    Var &operator=(const Var &other) { _data = other._data; }
    Var &operator=(const Var &&other) { _data = std::move(other._data); }

    DataType Get() { return _data; }
    std::vector<uint8_t> GetBytes() {
        return SuportType<T>::VarToBytes(&_data);
    }
    std::string GetAsString() {
        std::vector<uint8_t> ret = GetBytes();
        std::string tempRet(ret.begin(), ret.end());
        return tempRet;
    }
...
class HostCallerBase {
  public:
    virtual ~HostCallerBase() {}
    virtual void Call(const std::vector<std::vector<uint8_t>> &params) = 0;
    std::vector<uint8_t> GetResult() {
        auto ret = std::move(_resualt);
        _resualt.clear();
        return ret;
    }

  protected:
    std::vector<uint8_t> _resualt;
};

class TfscWasmtimeVM;

template <int... Seq> struct sequence {};

template <int N, int... Seq>
struct make_seq : make_seq<N - 1, N - 1, Seq...> {};

template <int... Seq> struct make_seq<0, Seq...> {
    using type = sequence<Seq...>;
};

template <typename RetType, typename... ParamsType>
class WASMTIMECaller : public HostCallerBase {
  public:
    WASMTIMECaller(const std::function<RetType(ParamsType...)> &cb) {
        callback = cb;
    }
    ~WASMTIMECaller() {}
    void Call(const std::vector<std::vector<uint8_t>> &params) override {
        if (params.size() != ParamSize) {
            errorL("param num error expect %s got %s", ParamSize,
                   params.size());
            return;
        }
        Var<RetType> ret = Call_detil(
            params, typename make_seq<sizeof...(ParamsType)>::type());
        _resualt = ret.GetBytes();
    }

  private:
    template <int... Seq>
    RetType Call_detil(const std::vector<std::vector<uint8_t>> &params,
                       const sequence<Seq...> &) {

        return callback((Var<ParamsType>(params[Seq]).Get())...);
    }
    std::function<RetType(ParamsType...)> callback;
    int ParamSize = sizeof...(ParamsType);
};

template <typename... ParamsType>
class WASMTIMECaller<void, ParamsType...> : public HostCallerBase {
  public:
    WASMTIMECaller(const std::function<void(ParamsType...)> &cb) {
        callback = cb;
    }
    ~WASMTIMECaller() {}
    void Call(const std::vector<std::vector<uint8_t>> &params) override {
        ...
        Call_detil(params, typename make_seq<sizeof...(ParamsType)>::type());
    }

  private:
    template <int... Seq>
    void Call_detil(const std::vector<std::vector<uint8_t>> &params,
                    const sequence<Seq...> &) {
        return callback((Var<ParamsType>(params[Seq]).Get())...);
    }
    std::function<void(ParamsType...)> callback;
    int ParamSize = sizeof...(ParamsType);
};

class WasmtimeVMhost {
  public:
    WasmtimeVMhost() {}

    void RegisterHostFunction(const std::string &funcName, HostCallerBase *nf) {
        hostFuncs[funcName] = nf;
    }
    ~WasmtimeVMhost() {
        for (auto &p : hostFuncs) {
            delete p.second;
        }
    }
    void SetHostError(const std::string & er,int errenum);

    wasmtime::_Error GetHostError();

  private:
    friend class TfscWasmtimeVM;
    std::map<std::string, HostCallerBase *> hostFuncs;
};

template <typename T>
static void InsterVMArgs(std::vector<std::vector<uint8_t>> &que, const T &t) {
    que.push_back(Var<T>(t).GetBytes());
}
...
static void InsterVMArgs(std::vector<std::vector<uint8_t>> &que) {}

class TfscWasmtimeVM {
  public:
    TfscWasmtimeVM(WasmtimeVMhost *hostCaller, const std::string &bytes)
        : _callInstance(hostCaller), _wasmBytes(bytes) {}
    template <typename... Ts>
  wasmtime::_Result<std::string> Call(const std::string &name, int64_t gas, const Ts &...ts) {
        InsterVMArgs(_inputParams, ts...);
     auto ret=   _WasmtimeInit(gas, _wasmBytes, name);
        _inputParams.clear();
        return ret;
    }
    wasmtime::_Result<std::string> Call(const std::string &name, int64_t gas,
                     const std::vector<std::string> &param_s) {
        int i = 0;
        for (auto &p : param_s) {
            _inputParams.push_back(
                std::vector<uint8_t>(param_s[i].begin(), param_s[i].end()));
            i++;
        }
        return _WasmtimeInit(gas, _wasmBytes, name);
    }
    int64_t GetConsumedCost() { return _consumedCost; }

  private:
    wasmtime::_Result<std::string> _WasmtimeInit(int64_t gas, const std::string &wasmbytes,
                       const std::string &name) {
        wasmtime::Config config;
        config.consume_fuel(true);
        wasmtime::Engine engine(std::move(config));
        std::vector<uint8_t> bytes(wasmbytes.begin(), wasmbytes.end());
        wasmtime::Module moudle = wasmtime::Module::compile(engine, bytes).ok();

        wasmtime::Store store(engine);
        store.context().add_fuel(gas).unwrap();
        ...
        wasmtime::Memory *cum = nullptr;

        linker
            .func_wrap("env", "tfsc_param",
                       [&](int32_t baseAddr, int32_t length) {
                           std::vector<uint8_t> param;
                           char p = cum->data(store)[baseAddr];
                           for (int i = 0; i < length; i++) {
                               param.push_back(
                                   (char)cum->data(store)[baseAddr + i]);
                           }
                           PushParam(param);
                       })
            .ok();
        ...
        linker
            .func_wrap("env", "tfsc_result_size",
                       [&]() -> int32_t { return _result.size(); })
            .ok();

        linker
            .func_wrap("env", "tfsc_result",
                       [&](int32_t baseAddr) {
                           for (int i = 0; i < _result.size(); i++) {
                               cum->data(store)[baseAddr + i] = _result[i];
                           }
                           Clear();
                       })
            .ok();
        ...
        linker
            .func_wrap("env", "tfsc_call",
                       [&](int32_t baseAddr, int32_t lenght) {
                           std::string funName;
        ...
        linker
            .func_wrap("env", "tfsc_set_run_retvalue",
                       [&](int32_t baseAddr, int32_t lenght) {
                           std::string ret;
                           char p = cum->data(store)[baseAddr];
                           for (int i = 0; i < lenght; i++) {
        ...
        linker
            .func_wrap("env", "tfsc_input_param_size",
                       [&](int index) -> int32_t {
                           if (index >= _inputParams.size()) {
                               debugL("index:%s out of paramSize:%s", index,
                                      _inputParams.size());
                               return 0;
                           }
                           return _inputParams[index].size();
                       })
            .ok();

        linker
            .func_wrap("env", "tfsc_input_param",
                       [&](int32_t index, int32_t baseAddr) {
                           auto ft = _inputParams[index];
                           wasmtime::Span<uint8_t> s = cum->data(store);
                           char *p = (char *)s.data();
                           p = p + baseAddr;
        ...
        auto instance = linker.instantiate(store, moudle).unwrap();
        auto memory =
            std::get<wasmtime::Memory>(*instance.get(store, "memory"));

        cum = &memory;

        auto run = std::get<wasmtime::Func>(*instance.get(store, name));

       auto res=run.call(store, {});

        _consumedCost = *store.context().fuel_consumed();
        auto Er=_callInstance->GetHostError();
        if(Er.er_n!=0){
            return Er;
        }
        return _RetValue;
    }
    void PushParam(const std::vector<uint8_t> &param) {
        _params.push_back(param);
    }
    void SetFunName(const std::vector<uint8_t> &fn) {
        Var<std::string> fN(fn);
        _funcName = fN.Get();
    }
    void Clear() {
        _params.clear();
    ...
    void Call() {
        ...
        auto iter = _callInstance->hostFuncs.find(_funcName);
        if (iter != _callInstance->hostFuncs.end()) {
            iter->second->Call(_params);
            _result = iter->second->GetResult();
        } else {
            std::cout << "error not found function " << _funcName << std::endl;
        }
    }

  private:
    std::string _wasmBytes;
    std::vector<std::vector<uint8_t>> _inputParams;
    std::vector<std::vector<uint8_t>> _params;
    std::vector<uint8_t> _result;
    int64_t _consumedCost;
    std::string _funcName;
    WasmtimeVMhost *_callInstance;
    std::string _RetValue;
};

class WasmStore {
  public:
    using ptr = std::shared_ptr<WasmStore>;

    WasmStore() { storeRoot = std::shared_ptr<Trie>(new Trie()); }
    void CreateTrie(const std::string &rootHash,
                    const std::string &ContractAddr) {


        if(rootHash.empty())
        {
            storeRoot = std::make_shared<Trie>(ContractAddr);
        }
        else
        {
            storeRoot = std::make_shared<Trie>(rootHash, ContractAddr);
        }
    }

    void CreateTrie(const std::string &ContractAddr) {
        storeRoot = std::shared_ptr<Trie>(new Trie(ContractAddr));
    }

    void InsterValue(const std::string &key, const std::string &value) {
        storeRoot->Update(key, value);
    }

    std::string GetValue(const std::string &key) {
        std::string k = key;
        return storeRoot->Get(k);
    }
    
  public:
    std::shared_ptr<Trie> storeRoot = nullptr;
};
void _wasm_time_init();

struct transation_target{
    std::string amount;
    std::string from;
    std::string to;
};

struct destruction_information
{
    std::string destoryAddr;
...
class StoreManager {
  public:
    ...
    void InsertStore(const std::string &contractAddr, WasmStore::ptr newStore) {
        auto iter = contractStores.find(contractAddr);
        if (iter != contractStores.end()) {
            for (const auto &account : contractStores) {
                std::shared_ptr<Trie> root = account.second->storeRoot;
                auto it = root.get()->root;
                if(it == nullptr)
                {
                    int a;
                }
        ...
        contractStores[contractAddr] = newStore;
        for (const auto &account : contractStores) {
            std::shared_ptr<Trie> root = account.second->storeRoot;
            auto it = root.get()->root;
            if(it == nullptr)
    ...
    WasmStore::ptr GetStore(const std::string &ContractAddr) {
        auto iter = contractStores.find(ContractAddr);
        if (iter != contractStores.end()) {
            return iter->second;
        } else {
            errorL("not found contract %s", ContractAddr);
            return nullptr;
        }
    }

    void GetStorage(nlohmann::json &jStorage,
        ...
        for (const auto &account : contractStores) {
                ...
                if (!rootHash.second.empty()) {
                    jStorage[root->contractAddr + "_" + rootHash.first] =
                        rootHash.second;
                }
    ...
    std::map<std::string, WasmStore::ptr> GetAllStoreMap(){
        return contractStores;
    }
   std::vector<transation_target> GetTrasationTarget(const std::string & contractAddr){
         std::vector<transation_target> ret;
        auto iter=contractTransations.find(contractAddr);
        if(iter==contractTransations.end()){
            return std::vector<transation_target>();
        }
        ret=iter->second;
        iter->second.clear();
        return ret;
    ...
    void InstertTransationTarget(const std::string & contractAddr, const transation_target & tra){
        contractTransations[contractAddr].push_back(tra);
    }

    void RemoveTransationTarget(const std::string & contractAddr){
        auto iter=contractTransations.find(contractAddr);
        if(iter != contractTransations.end()){
            contractTransations.erase(iter);
        }
    }

    std::string getCurrentContractAddr() const {
        return currentContractAddr;
    }

    void setCurrentContractAddr(const std::string& contractAddr) {
        currentContractAddr = contractAddr;
    }

    std::vector<destruction_information> destruction_data; 
  private:
    std::string currentContractAddr;
    std::map<std::string ,std::vector<transation_target>> contractTransations; 
    std::map<std::string, WasmStore::ptr> contractStores;
};

template <typename RetType, typename... ParamsType> class MakeHost {
  public:
    MakeHost(TFSC::WasmtimeVMhost &Call, const std::string &name,
             const std::function<RetType(ParamsType...)> &bc) {
        TFSC::WASMTIMECaller<RetType, ParamsType...> *host =new TFSC::WASMTIMECaller<RetType, ParamsType...>(bc);
        Call.RegisterHostFunction(name, host);
    }
};
