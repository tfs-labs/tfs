...
Account::Account() : _pkey(nullptr, &EVP_PKEY_free)
{
    ...
    _GeneratePubStr();
    _GeneratePriStr();
    _GenerateBase58Addr(Base58Ver::kBase58Ver_Normal);
...
Account::Account(Account&& other) noexcept :
    ...
    _base58Addr(std::move(other._base58Addr))
{
}
...
void Account::_GeneratePubStr()
{
    ...
    int publen = i2d_PUBKEY(_pkey.get() ,&pkeyDer);
...
void Account::_GeneratePriStr()
{
    size_t len = 80;
    char pkeyData[80] = {0};
    if( EVP_PKEY_get_raw_private_key(_pkey.get(), (unsigned char *)pkeyData, &len) == 0)
    {
        return;
    }

    std::string data(pkeyData, len);
    _priStr = data;
}


void Account::_GenerateBase58Addr(Base58Ver ver)
{
    _base58Addr = GetBase58Addr(_pubStr, ver);
}
...
void AccountManager::PrintAllAccount() const
{
    ...
    {
        if (iter->first == _defaultBase58Addr)
        {
...
{
    if(base58addr == _defaultBase58Addr)
    {
...
{
    _defaultBase58Addr = bs58Addr;
}

std::string AccountManager::GetDefaultBase58Addr() const
{
    return _defaultBase58Addr;
}
...
int AccountManager::SetDefaultAccount(const std::string & bs58Addr)
{
    ...
    if (bs58Addr.size() == 0)
    {
        _defaultBase58Addr = _accountList.begin()->first;
        return 0;
    }
...
{
    auto iter = _accountList.find(_defaultBase58Addr);
    if(iter == _accountList.end())
    {
        ERRORLOG("not found DefaultKeyBs58Addr {} in the _accountList ", _defaultBase58Addr);
        return -1;
    }
    account = *iter->second;
...
int AccountManager::ImportMnemonic(const std::string & mnemonic)
{
    ...
    ImportPrivateKeyHex(mnemonicKey);
